script_version('2')
script_name("BETAAttractTools")
-- script v is "14.12.2025"
...
local cfg = inicfg.load({
 ...
 = true,
		...
		nead = 60,
		mdm = 5,
		sendgoodplay = tostring(u8'/msg Администрация Attract RP желает Вам приятной игры на просторах нашего сервера!'),
		cmdHouse = 'ohome',
		cmdBusiness = 'obiz',
		...
		infopunish1 = tostring(u8'Ссылочка на наш форум - *** https://forum.attract-rp.ru *** | Приятной игры!'),
		kapchas = '',
		present = '',
		formis = '',
		akkpass = '',
...
forma_offwarn = imgui.ImBool(cfg.formssettings.forma_offwarn)
...
forma_offjail = imgui.ImBool(cfg.formssettings.forma_offjail)
forma_offmute = imgui.ImBool(cfg.formssettings.forma_offmute)
...
forma_offban = imgui.ImBool(cfg.formssettings.forma_offban)
...
forma_unoffjail = imgui.ImBool(cfg.formssettings.forma_unoffjail)
...
forma_unoffmute = imgui.ImBool(cfg.formssettings.forma_unoffmute)
...
autoforma_offwarn = imgui.ImBool(cfg.formssettings.autoforma_offwarn)
...
autoforma_offjail = imgui.ImBool(cfg.formssettings.autoforma_offjail)
autoforma_offmute = imgui.ImBool(cfg.formssettings.autoforma_offmute)
...
autoforma_offban = imgui.ImBool(cfg.formssettings.autoforma_offban)
...
autoforma_unoffjail = imgui.ImBool(cfg.formssettings.autoforma_unoffjail)
...
autoforma_unoffmute = imgui.ImBool(cfg.formssettings.autoforma_unoffmute)
...
notf_offwarn = imgui.ImBool(cfg.formssettings.notf_offwarn)
...
notf_offjail = imgui.ImBool(cfg.formssettings.notf_offjail)
notf_offmute = imgui.ImBool(cfg.formssettings.notf_offmute)
...
notf_offban = imgui.ImBool(cfg.formssettings.notf_offban)
...
notf_unoffjail = imgui.ImBool(cfg.formssettings.notf_unoffjail)
...
notf_unoffmute = imgui.ImBool(cfg.formssettings.notf_unoffmute)
...
function main()
	...
	while server ~= "217.106.106.22:7777" do wait(15);
		sampAddChatMessage("TOOLS ATTRACT DOES NOT WORK ON THIS SERVER, ONLY ON ATTRACT: 217.106.106.22:7777", -1);
...
local function get_id_by_nick(nick)
	...
	if cfg.main.gm then
		...
		if cfg.main.airbrake then 
			...
			printStringNow("~w~attract "..(enAirBrake and "~b~OFF" or "~g~ON"), 2000)
		...
		if isKeysDown(facceptform.v) and not sampIsChatInputActive() and not sampIsDialogActive() and not isPauseMenuActive() and not isSampfuncsConsoleActive() and cfg.main.trueform then
			...
			lua_thread.create(function()
				...
				autoformaq = "false"
				wait (502)
				sampSendChat("/a [Attract-TOOLS] ~ Forma [+]")
				wait (502)
				sampAddChatMessage("{00BFFF}[Attract-TOOLS] - Успешно принятая форма от: " .. admin_nick .. "{FFFFFF}")	
				acceptforma = "true"
			end)
			end
		end
...
local X, Y = getScreenResolution()
	...
	if MainWindow.v then
		...
		imgui.Begin(fa.ICON_FA_TOOLS..u8' ATTRACT RP | TOOLS '..fa.ICON_FA_TOOLS, MainWindow, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoSavedSettings) 	
	   ...
	   imgui.Hint('saves', u8'Сохранить настройки')
		...
		if menusel.v == 1 then
			...
			if imgui.ToggleButton(u8'Логгирование выдачи блокировок в оффлайне', imgui.ImBool(cfg.main.logoffban)) then 
				cfg.main.logoffban = not cfg.main.logoffban 
			end 
			if imgui.ToggleButton(u8'Логгирование выдачи деморганов в оффлайне', imgui.ImBool(cfg.main.logoffjail)) then 
				cfg.main.logoffjail = not cfg.main.logoffjail 
			end 
			if imgui.ToggleButton(u8'Логгирование выдачи заглушек в оффлайне', imgui.ImBool(cfg.main.logoffmute)) then 
				cfg.main.logoffmute = not cfg.main.logoffmute 
			end 
...
imgui.Separator()
	...
	if imgui.Button(u8("Добавить кнопку ответа в репорт"), imgui.ImVec2(-1, 25)) then
		...
		if menusel.v == 13 then
			...
			imgui.PopFont()
			imgui.PushFont(fs80)
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize('MASHERA CODER').x) / 2)
			imgui.TextColored(imgui.ImVec4(rainbowtext(4)), 'MASHERA CODER')
			imgui.PopFont()
			imgui.PushFont(fs16)
			imgui.CenterTextColoredRGB('Реализовано для проекта:')
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize('ATTRACT RP').x) / 2 - 65)
			imgui.TextColored(imgui.ImVec4(rainbowtext(4)), 'ATTRACT RP | vk.com/vladiksng')
			imgui.PopFont()
			imgui.PushFont(fs16)
...
imgui.RadioButton(u8"В общий чат (/msg)", when_captcha, 1)
imgui.RadioButton(u8"В VIP чат (/v)", when_captcha, 2)
imgui.Text(u8'Капча:')
imgui.SameLine()
imgui.PushItemWidth(-1) 
...
imgui.BeginChild("qeuqewiqweiqwe5", imgui.ImVec2(-1, 60), true)
		...
		imgui.CenterText(u8('/v Внимание! Капча: "'..u8:decode(cfg.main.kapchas))..u8('". Приз: '..u8:decode(cfg.main.present))..'!')
		else
		imgui.CenterText(u8('/msg Внимание! Капча: "'..u8:decode(cfg.main.kapchas))..u8('". Приз: '..u8:decode(cfg.main.present))..'!')
		end
imgui.EndChild()
if imgui.Button(u8'Отправить капчу', imgui.ImVec2(-1, 25)) then
if when_captcha.v == 2 then
sampSendChat('/v Внимание! Капча: "'..u8:decode(cfg.main.kapchas)..'". Приз: '..u8:decode(cfg.main.present)..'!')
else
sampSendChat('/msg Внимание! Капча: "'..u8:decode(cfg.main.kapchas)..'". Приз: '..u8:decode(cfg.main.present)..'!')
end
end
imgui.End()
	end
...
if imgui.Button(u8'Указать свою причину', imgui.ImVec2(-1, 20)) then
   sampSetChatInputEnabled(true)
   sampAddChatMessage(tag..'На Attract RP данная функция не работает!', -1)
   imgui.CloseCurrentPopup()
end
if imgui.Button(u8'Закрыть', imgui.ImVec2(140, 20)) then
imgui.CloseCurrentPopup()
...
lua_thread.create(function()
sampSendChat('/msg [Спавн Транспорта] Уважаемые игроки, через '..arg..' секунд произойдет респавн всего транспорта.')
sampSendChat('/msg [Спавн Транспорта] Займите свой транспорт, в противном случае он пропадет!')
for i = arg,1,-1 do
wait(1000)
printStyledString("Spawn cars through: ~r~"..i, 1000, 5)
		...
		spcars = true
		sampSendChat('/apanel')
sampSendChat('/msg [Спавн Транспорта] Спавн транспорта произошел успешно | Приятной игры!')		
	end)
	else
...
if text:find("%(%( Администратор (.*)%[(%d+)%]: {AFAFAF}(.*) {FFE6E6}%)%)") then
	...
	if cfg.main.formaplus then 
		...
		if text:find("/offmute %[nick%] %[time%] %[reason%]") and acceptforma then
			sampSendChat("/a [Forma] ".. admin_nick .. ", используйте: /offmute [nick] [time] [reason]")
			acceptforma = false
		end
...
if text:find("Администратор (.*) забанил в оффлайне игрока (.*)%. Причина: (.*)") and cfg.main.logoffban then
adminnick, playernick, reason = text:match("Администратор (.*) забанил в оффлайне игрока (.*)%. Причина: (.*)")
local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Логи//offbanLogs.txt", 'a')
f:write('['..os.date("%d.%m.%Y %X", os.time())..'] Администратор '..adminnick..' заблокировал в оффлайне игрока '..playernick..'. Причина: '..reason..'\n')
f:close()
end

if text:find("Администратор (.*) установил в оффлайне (%d+) минут тюрьмы игроку (.*)%. Причина: (.*)") and cfg.main.logoffjail then
adminnick, srok, playernick, reason = text:match("Администратор (.*) установил в оффлайне (%d+) минут тюрьмы игроку (.*)%. Причина: (.*)")
local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Логи//offjailLogs.txt", 'a')
...
if text:find("Администратор (.*) установил в оффлайне (%d+) минут молчанки игроку (.*)%. Причина: (.*)") and cfg.main.logoffmute then
adminnick, srok, playernick, reason = text:match("Администратор (.*) установил в оффлайне (%d+) минут молчанки игроку (.*)%. Причина: (.*)")
local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Логи//offmuteLogs.txt", 'a')
...
local reasons = {"deladmtag", "rmute", "unrmute", "slap", "flip", "freeze", "unfreeze", "pm", "spplayer", "sethp", "unjail", "jail", "weap", "unmute", "mute", "spcar", "kick",
	"getip", "pgetip", "plveh", "unban", "ban", "unwarn", "warn", "spcars", "givegun", "removetune", "delbname", "delhname", "offwarn", "setgangzone", "makeleader", "sban", "banip", "unbanip",
	"offjail","offmute","skick","setskin","uval","ao","offban","agl","setname","veh","agiveskin","setadmtag","giveitem","acceptadmin","awarn","unoffjail", "asellbiz", "asellhouse", "setarmour", "unoffmute"}	
		...
		if text:match('%[.*%] (%w+_?%w+)%[(%d+)%]%: /'..v..'%s') and cfg.main.trueform then
			...
			if not isGamePaused() then
				...
				or cmd == "offwarn" and cfg.formssettings.autoforma_offwarn
				...
				or cmd == "unbanip" and cfg.formssettings.autoforma_unbanip or cmd == "offjail" and cfg.formssettings.autoforma_offjail 
				or cmd == "offmute" and cfg.formssettings.autoforma_offmute or cmd == "skick" and cfg.formssettings.autoforma_skick 
				...
				or cmd == "offban" and cfg.formssettings.autoforma_offban or cmd == "agl" and cfg.formssettings.autoforma_agl 
				...
				or cmd == "awarn" and cfg.formssettings.autoforma_awarn or cmd == "unoffjail" and cfg.formssettings.autoforma_unoffjail 
				or cmd == "asellbiz" and cfg.formssettings.autoforma_asellbiz or cmd == "asellhouse" and cfg.formssettings.autoforma_asellhouse 
				or cmd == "setarmour" and cfg.formssettings.autoforma_setarmour or cmd == "unoffmute" and cfg.formssettings.autoforma_unoffmute then
						lua_thread.create(function()
						...
						or cmd == "offwarn" and cfg.formssettings.notf_offwarn
						...
						or cmd == "unbanip" and cfg.formssettings.notf_unbanip or cmd == "offjail" and cfg.formssettings.notf_offjail 
						or cmd == "offmute" and cfg.formssettings.notf_offmute or cmd == "skick" and cfg.formssettings.notf_skick 
						...
						or cmd == "offban" and cfg.formssettings.notf_offban or cmd == "agl" and cfg.formssettings.notf_agl 
						...
						or cmd == "awarn" and cfg.formssettings.notf_awarn or cmd == "unoffjail" and cfg.formssettings.notf_unoffjail 
						or cmd == "asellbiz" and cfg.formssettings.notf_asellbiz or cmd == "asellhouse" and cfg.formssettings.notf_asellhouse 
						or cmd == "setarmour" and cfg.formssettings.notf_setarmour or cmd == "unoffmute" and cfg.formssettings.notf_unoffmute then
							playVolume()
						end	
end						
				else
	...
	for k,v in ipairs(reasons) do
		if text:match("%[.*%] (%w+_?%w+)%[(%d+)%]%: /"..v.."%s") and cfg.main.trueform then
			...
			admin_nick, admin_id, other = text:match("%[.+%] (%w+_?%w+)%[(%d+)%]%: /"..v.."%s(.*)")
				...
				or cmd == "offwarn" and cfg.formssettings.forma_offwarn
				...
				or cmd == "unbanip" and cfg.formssettings.forma_unbanip or cmd == "offjail" and cfg.formssettings.forma_offjail 
				or cmd == "offmute" and cfg.formssettings.forma_offmute or cmd == "skick" and cfg.formssettings.forma_skick 
				...
				or cmd == "offban" and cfg.formssettings.forma_offban or cmd == "agl" and cfg.formssettings.forma_agl 
				...
				or cmd == "awarn" and cfg.formssettings.forma_awarn or cmd == "unoffjail" and cfg.formssettings.forma_unoffjail 
				or cmd == "asellbiz" and cfg.formssettings.forma_asellbiz or cmd == "asellhouse" and cfg.formssettings.forma_asellhouse 
				or cmd == "setarmour" and cfg.formssettings.forma_setarmour or cmd == "unoffmute" and cfg.formssettings.forma_unoffmute then
...
if not isGamePaused() then	
	...
	if text:find("%[A] (Sasha_Gozhelnikov)%[(%d+)%]: bot check updates") then
		...
		anickname, aid = text:match("%[A] (Sasha_Gozhelnikov)%[(%d+)%]: bot obnova")
				sampSendChat('/a новая версия скрипта, установить: vk.com/vladiksng')
	end
	...
	if text:find("(.*) %[(.*)%] купил бизнес ID: (.*) по гос. цене за (.*) ms! Капча: %((%d+) | (%d+)%)") and cfg.autoopra.settingbiz then
		...
		if cfg.autoopra.myreasonbiz then
			...
			if not result then
				send_rpc_command('/offjail '..bnickp..' '..cfg.main.timeopra..' '..template)
			else
		...
		local result = sampIsPlayerConnected(bidp)
			if not result then
				send_rpc_command('/offjail '..bnickp..' '..cfg.main.timeopra..' опра бизнес №'..bidbiz)
			else
	...
	if text:find("(.*) %[(.*)%] купил дом ID: (.*) по гос. цене за (.*) ms! Капча: %((%d+) | (%d+)%)") and cfg.autoopra.settinghouse then
		...
		if cfg.autoopra.myreasonhouse then
			...
			if not result then
				send_rpc_command('/offjail '..nickp..' '..cfg.main.timeopra..' '..template)
			else
...
function kotofeevformi()
	...
	imgui.Button(u8'/offjail', imgui.ImVec2(-1, 20))
	imgui.Button(u8'/offmute', imgui.ImVec2(-1, 20))
	imgui.Button(u8'/skick', imgui.ImVec2(-1, 20))
	imgui.Button(u8'/setskin', imgui.ImVec2(-1, 20))
	imgui.Button(u8'/uval', imgui.ImVec2(-1, 20))
	imgui.Button(u8'/msg', imgui.ImVec2(-1, 20))
	...
	imgui.Button(u8'/unoffjail', imgui.ImVec2(-1, 20))
	...
	imgui.Button(u8'/unoffmute', imgui.ImVec2(-1, 20))
	...
	if imgui.Checkbox('##setadmtag', forma_setadmtag) then
		...
		if imgui.Checkbox('##offwarn', forma_offwarn) then
			cfg.formssettings.forma_offwarn = forma_offwarn.v
		end
		...
		if imgui.Checkbox('##offjail', forma_offjail) then
			cfg.formssettings.forma_offjail = forma_offjail.v
		end
	
	
		if imgui.Checkbox('##offmute', forma_offmute) then
			cfg.formssettings.forma_offmute = forma_offmute.v
		end
		...
		if imgui.Checkbox('##offban', forma_offban) then
			cfg.formssettings.forma_offban = forma_offban.v
		end
		...
		if imgui.Checkbox('##unoffjail', forma_unoffjail) then
			cfg.formssettings.forma_unoffjail = forma_unoffjail.v
		end
	...
	if imgui.Checkbox('##unoffmute', forma_unoffmute) then
		cfg.formssettings.forma_unoffmute = forma_unoffmute.v
	end
	...
	if imgui.Checkbox('##set1admtag', autoforma_setadmtag) then
		...
		if imgui.Checkbox('##w1arnoff', autoforma_offwarn) then
			cfg.formssettings.autoforma_offwarn = autoforma_offwarn.v
		end
		...
		if imgui.Checkbox('##jailo1ff', autoforma_offjail) then
			cfg.formssettings.autoforma_offjail = autoforma_offjail.v
		end
		
	
		if imgui.Checkbox('##muteo1ff', autoforma_offmute) then
			cfg.formssettings.autoforma_offmute = autoforma_offmute.v
		end
		...
		if imgui.Checkbox('##bano1ff', autoforma_offban) then
			cfg.formssettings.autoforma_offban = autoforma_offban.v
		end
		...
		if imgui.Checkbox('##unj1ailoff', autoforma_unoffjail) then
			cfg.formssettings.autoforma_unoffjail = autoforma_unoffjail.v
		end
	...
	if imgui.Checkbox('##unmut1eoff', autoforma_unoffmute) then
		cfg.formssettings.autoforma_unoffmute = autoforma_unoffmute.v
	end
	...
	if imgui.Checkbox('##set1admtag', notf_setadmtag) then
		...
		if imgui.Checkbox('##w1arnoff', notf_offwarn) then
			cfg.formssettings.notf_offwarn = notf_offwarn.v
		end
		...
		if imgui.Checkbox('##jailo1ff', notf_offjail) then
			cfg.formssettings.notf_offjail = notf_offjail.v
		end
		
	
		if imgui.Checkbox('##muteo1ff', notf_offmute) then
			cfg.formssettings.notf_offmute = notf_offmute.v
		end
		...
		if imgui.Checkbox('##bano1ff', notf_offban) then
			cfg.formssettings.notf_offban = notf_offban.v
		end
		...
		if imgui.Checkbox('##unj1ailoff', notf_unoffjail) then
			cfg.formssettings.notf_unoffjail = notf_unoffjail.v
		end
	...
	if imgui.Checkbox('##unmut1eoff', notf_unoffmute) then
		cfg.formssettings.notf_unoffmute = notf_unoffmute.v
	end
...
if cfg.autoopra.myreasonbiz then
		...
		local result = sampIsPlayerConnected(idp)
			if not result then
				send_rpc_command('/offjail '..nickp..' '..cfg.main.timeopra..' опра бизнес №'..idbiz)
			else
...
if not doesDirectoryExist(getGameDirectory()..'//moonloader//AdminToolsKing//Логи//') then 
	...
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//zawitabinder.txt") then
		...
		f:write(u8'/msg Уважаемые игроки, берегите свои аккаунты. Устанавливайте привязку по E-Mail!\n/msg Не давайте пароль от своего аккаунта никому, не скачивайте модификации с неизвестных сайтов.\n/msg Напомню, администрация не несет ответственности за ваши аккаунты. Все в ваших руках!')
		f:close()
	...
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//donatebinder.txt") then
		...
		f:write(u8'/msg Уважаемые игроки, на данный момент на сервере действует акция X3 донат!\n/msg Вся пополненная сумма увеличивается в 3 раза, не упустите возможность приобрести привилегии по низкой цене!\n/msg Пополнение счета на нашем сайте - /info')
		f:close()
	...
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//rulesbinder.txt") then
		...
		f:write(u8'/msg Уважаемые игроки, советую ознакомиться с правилами сервера на нашем форуме - /info\n/msg За нарушение правил сервера Вы будете строго наказаны, приятной игры!')
		f:close()
	...
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//discordbinder.txt") then
		...
		f:write(u8'/msg Не хватает общения? Хочешь продать имущество? После сложного учебного дня хочется отдохнуть?\n/msg Тогда присоединяйся к Беседе нашего сервера! Адекватные игроки и модерация.\n/msg Ссылка на нашу беседу: /info')
		f:close()
	...
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//groupbinder.txt") then
		...
		f:write(u8'/mssg Уважаемые игроки, хотите узнавать актуальную информацию самым первым?\n/msg Для такого случая у нас есть паблик ВК - /info, узнавайте новости самым первым!')
		f:close()
	...
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//Логи//offbanLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Логи//offbanLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи блокировок в оффлайне создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//Логи//offjailLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Логи//offjailLogs.txt", 'w')
		f:close()
		sampAddChatMessage(tag..'Файл для записи выдачи деморганов в оффлайне создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//AdminToolsKing//Логи//offmuteLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//AdminToolsKing//Логи//offmuteLogs.txt", 'w')
		f:close()
...
sampAddChatMessage(tag..'Укажите верный NickName игрока!', -1)
	...
	local result = sampIsPlayerConnected(id)
	if not result then
		send_rpc_command('/unoffjail '..arg..' опровержение принято')
	else
...
sampAddChatMessage(tag..'Укажите верный NickName игрока!', -1)
	...
	local result = sampIsPlayerConnected(id)
	if not result then
		send_rpc_command('/unoffjail '..arg..' опровержение отклонено')
	else
...
sampAddChatMessage(tag..'Разработчик: {FF69B4}mashera *masherka', -1)
sampAddChatMessage(tag..'Для получения дополнительной информации по скрипту, посетите сайт: {FF69B4}vk.com/vladiksng', -1)
sampAddChatMessage(tag..'Этот скрипт работает только на: {FF69B4}ATTRACT RP', -1)
end
